...
app=FastAPI(title=trying to connect binance websocet)
...
@app.websocet("/ws")
async def websocket_endpoint(websocet:WebSocket):
    await manager.connect(websocet)
    try:
        while True:
            data=await websocet.receive_text()
            try:
                msg=json.loads(data)

                if manager.binance_ws and manager.is_connected:
                    await websocet.send_json({"type":"forwarded"})
